...
from util.torchtool import load_pretrained_weights, load_checkpoint
...
class ResNet(nn.Module):
    ...
    def forward(self, x: Tensor, feature_only=False) -> Tensor:
...
def _resnet(
    ...
    if pretrained:
        print(f'loading pretrained model from url:{model_urls[arch]}')
...
available_backbones = list(backbones.keys())

if __name__ == '__main__':
    m1 = resnet18(True)
    m2 = resnet18(False)
    state_dict = load_checkpoint(r'../resnet18-imagenet.pth')
    load_pretrained_weights(m2, state_dict)

    for x,y in zip(m1.parameters(), m2.parameters()):
        print((x == y).all())

    # print(m1.conv1.weight.data)
    # print(m2.conv1.weight.data)
    # x1 = torch.zeros((2,3))
    # x2 = torch.zeros((2,3))
    # x1[0,0]=5
    # print((x1 == x2).all())
    # print((m1.conv1.weight.data == m2.conv1.weight.data).all())
    pass
